...
from src.common.db import executemany, return_scalar, with_cursor
from src.common.utils import (
    ensure_env_loaded,
    get_env_int,
    get_env_str,
    numpy_uint8_to_bytes,
    sha256_bytes,
)
...
def insert_rows(snapshot_id, table_name, X, y, batch_size=5000):
    ...
    for start in range(0, n, batch_size):
        ...
        batch = [
            (int(snapshot_id), int(lbl), numpy_uint8_to_bytes(row))
            for row, lbl in zip(X[start:end], y[start:end])
        ]
...
def main():
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=TEST_SIZE, random_state=EXTRACT_RANDOM_SEED, stratify=y
    )
    ...
    if not existing:
        ...
        snapshot_sql = """
            ...
            RETURNING id
        """
        snapshot_id = return_scalar(
            snapshot_sql, (DATA_SOURCE, rows_train, rows_test, train_hash, test_hash)
        )
    ...
    summary = {
        "message": (
            "Dataset unchanged; reusing existing snapshot"
            if existing
            else "Created new snapshot"
        ),
