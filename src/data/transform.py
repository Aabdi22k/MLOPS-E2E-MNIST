...
from src.common.db import return_scalar, return_all, executemany, latest_snapshot_id
from src.common.utils import get_env_int, get_env_str, get_env_float, ensure_env_loaded
...
def upsert_split_rows(split_configuration_id, ids_train, ids_val):
    ...
    """
    params = (
        [(split_configuration_id, int(i), 'train') for i in ids_train] + 
...
def main():
    ...
    sss = StratifiedShuffleSplit(n_splits=1, test_size=VAL_FRACTION, random_state=TRANSFORM_RANDOM_SEED)
    ...
    split_configuration_id = upsert_split_configurations(snapshot_id, SPLIT_NAME, params, stats)
