...
def test_counts_match_snapshot(conn):
    ...
    cnt_train = scalar(conn, "SELECT COUNT(*) FROM mnist_images_train WHERE snapshot_id = %s", (snap["id"],))
    cnt_test  = scalar(conn, "SELECT COUNT(*) FROM mnist_images_test  WHERE snapshot_id = %s", (snap["id"],))
...
def test_label_range(conn):
    snap = latest_snapshot(conn)
    min_train = scalar(conn, "SELECT MIN(label) FROM mnist_images_train WHERE snapshot_id = %s", (snap["id"],))
    max_train = scalar(conn, "SELECT MAX(label) FROM mnist_images_train WHERE snapshot_id = %s", (snap["id"],))
    min_test  = scalar(conn, "SELECT MIN(label) FROM mnist_images_test  WHERE snapshot_id = %s", (snap["id"],))
    max_test  = scalar(conn, "SELECT MAX(label) FROM mnist_images_test  WHERE snapshot_id = %s", (snap["id"],))
...
def test_no_nulls(conn):
    snap = latest_snapshot(conn)
    n1 = scalar(conn, "SELECT COUNT(*) FROM mnist_images_train WHERE snapshot_id = %s AND (label IS NULL OR pixels IS NULL);", (snap["id"],))
    n2 = scalar(conn, "SELECT COUNT(*) FROM mnist_images_test  WHERE snapshot_id = %s AND (label IS NULL OR pixels IS NULL);", (snap["id"],))
